...
public class AccountProxyImpl implements AccountProxy {
  ...
  private Mono<? extends Throwable> applyError4xx(ClientResponse creditResponse, String errorMessage) {
    ...
    if (creditResponse.statusCode().equals(HttpStatus.NOT_FOUND)) {
      return Mono.error(new AccountNotFoundException(errorMessage, creditResponse.statusCode().value()));
    }
    return creditResponse.bodyToMono(String.class)
            .flatMap(response -> Mono.error(new AccountNotFoundException(response, creditResponse.statusCode().value())));
  }
