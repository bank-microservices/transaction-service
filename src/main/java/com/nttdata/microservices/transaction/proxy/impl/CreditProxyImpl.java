...
public class CreditProxyImpl implements CreditProxy {
  ...
  public Mono<CreditCard> findCreditCardByAccountNumber(String accountNumber) {
    String errorMessage = String.format("There is CreditCard not found with accountNumber: %s ", accountNumber);
  ...
  private Mono<? extends Throwable> applyError4xx(ClientResponse creditResponse, String errorMessage) {
    ...
    if (creditResponse.statusCode().equals(HttpStatus.NOT_FOUND)) {
      return Mono.error(new CreditNotFoundException(errorMessage, creditResponse.statusCode().value()));
    }
    return creditResponse.bodyToMono(String.class)
            .flatMap(response -> Mono.error(new CreditNotFoundException(response, creditResponse.statusCode().value())));
  }
